	...
	while( (c= (int)getc(file)) != EOF)
	   {
		...
		/*symbol commands*/
			case IGL_SYMBOL:
				isym = getc(file);
				getxy(&x,&y,file);
				size= (float)(geti(file))/FLOATNORM;
				angle= 360.0*(float)(geti(file))/ANGLENORM;
...
FILE *file;
   {
	...
	extern int page;
	register struct xypack { int xin, yin; } xy;
	...
	case 4: 
		...
		break;
	default:
		err(FATAL,"Unsupported INTYTES = %d", intbytes);
	}
