...
//      fprintf("stderr","%4s,%.0f  Max Amp=%.2e cm \n",ss[1].name,ss[1].azi*180.0/PI,scalefac);

//   for(j=0 ; j < ss->nn ; j++)
   fprintf(stderr,"Here OK____:%f \n",ss[1].t[j]*1000000000);
  ...
  fprintf(stderr,"Mo=%g\nMw=%.1f\nStrike=%.0f ; %.0f\nRake=%.0f ; %.0f\nDip=%.0f; %.0f\nPdc=%.0f\nPclvd=%.0f\nPiso=%.0f\n",
   ...
   for(i=0; i < nsta; i++)
   {
      ...
      sprintf(str,"%4s  azi %.0f  Max Amp=%.2e cm\n",ss[i].name,ss[i].azi*180.0/PI,scalefac);
//      fprintf(stderr,"Here ok: %s\n",str);
      text(0.2,y-0.50,str); 
...
//      text(0.2,y-0.65,"VR=%.1f",ss[i].vr);
	 ...
	 tmp[j]= (M.mxx*0.5*gg[i].u8[j+Zg] 
      ...
      sprintf(str,"Strike=%.0f ; %0.f",Strike,St2);
      text(8.0,7.0,str);

      sprintf(str,"Rake  =%.0f ; %0.f",Rake,Rk2);
      text(8.0,6.7,str);

      sprintf(str,"Dip   =%.0f ; %0.f",Dip,Dp2);
      text(8.0,6.4,str);

      sprintf(str,"Mo    =%.2e",Mo);
      text(8.0,6.1,str);

      sprintf(str,"Mw    =%.1f",Mw);
      text(8.0,5.8,str);

      sprintf(str,"Percent DC=%.0f",Pdc);
      text(8.0,5.5,str);

      sprintf(str,"Percent CLVD=%.0f",Pclvd);
      text(8.0,5.2,str);

      sprintf(str,"Percent ISO=%.0f",Piso);
      text(8.0,4.9,str);

      sprintf(str,"Variance=%.2e",E);
      text(8.0,4.6,str);

      sprintf(str,"Var. Red=%.2e",VR);
      text(8.0,4.3,str);

      sprintf(str,"RES/Pdc.=%.2e",E/Pdc);
      text(8.0,4.0,str);
...
if(kspin==1){
	...
	plot(xp,yp+0.05,1);
	spin=spin/2.0;
    sprintf(str,"%4.2f sec",spin);
	text(xp-0.525,yp-0.2,str);
	}
}
